...
describe("clearCustodyTx", () => {
  ...
  it("rejects a non-owner caller with OwnableUnauthorizedAccount", async function () {
  ...
  it("reverts CustodyTxClearNonceOutOfRange when nonce == counter", async function () {
    const network = await ethers.provider.getNetwork();
    if (network.name !== "hardhat" && network.name !== "unknown") this.skip();
  ...
  it("reverts CustodyTxClearMissingVouch for MarkSuccessWithHash with zero vouch", async function () {
    const network = await ethers.provider.getNetwork();
    if (network.name !== "hardhat" && network.name !== "unknown") this.skip();
  ...
  it("reverts CustodyTxClearUnexpectedVouch for non-MarkSuccess actions with a non-zero vouch", async function () {
    const network = await ethers.provider.getNetwork();
    if (network.name !== "hardhat" && network.name !== "unknown") this.skip();
  ...
  it("first-clear-wins: a second clear reverts CustodyTxAlreadyCleared with the stored hash", async function () {
    const network = await ethers.provider.getNetwork();
    if (network.name !== "hardhat" && network.name !== "unknown") this.skip();
...
describe("signNonceBurn", () => {
  ...
  it("reverts CustodyTxClearNotBurnAuthorized when the slot is uncleared", async function () {
    const network = await ethers.provider.getNetwork();
    if (network.name !== "hardhat" && network.name !== "unknown") this.skip();
  ...
  it("reverts CustodyTxClearNotBurnAuthorized when the slot was cleared with a non-BurnNonce action", async function () {
    const network = await ethers.provider.getNetwork();
    if (network.name !== "hardhat" && network.name !== "unknown") this.skip();
  ...
  it("reverts GasPriceNotSet when the slot is BurnNonce-cleared but no gas price is set", async function () {
    const network = await ethers.provider.getNetwork();
    if (network.name !== "hardhat" && network.name !== "unknown") this.skip();
