// Unit tests for the 05.07 helpers (`buildBridgeManifest`,
// `reconcileManifest`, `writeManifestAtPath`/`writeBridgeManifestAtomic`)
// plus the `XERC20_SOURCE` ↔ README parity check. All tests are pure /
// filesystem-only — no Hardhat fixture or contract deploys required.
//
// Spec: docs/plans/rose-bridge-phase0/05-createx-deploy/07-write-deployment-manifest.md
...
describe("DeployBridgeManifest", () => {
  ...
  describe("reconcileManifest", () => {
    ...
    it("throws naming the field on xrose mismatch (Red expectation)", () => {
      const existing = buildExpectedManifest();
      const proposed = buildExpectedManifest({
  ...
  describe("writeBridgeManifestAtomic", () => {
    // Writes to the canonical `solidity/deployments/bridge-<network>.json` path.
    // Tests use a one-off network name and clean up afterward so the suite never
    // pollutes a real deployment record. The wrapper is essentially
    // `writeManifestAtPath(manifestPath(network), ...)`.
