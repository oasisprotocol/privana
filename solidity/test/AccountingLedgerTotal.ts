...
// Must equal getTokenId(BridgeAsset, encodeBridgeAssetTokenData("ROSE")).
...
describe('AccountingLedgerTotal — ROSE_TOKEN_ID constant + ledger storage', () => {
  // Checks the bytes32 public constant ROSE_TOKEN_ID,
  ...
  it('ROSE_TOKEN_ID() returns the pinned literal', async () => {
...
describe('AccountingLedgerTotal — _increaseLedgerTotal wiring on creditDeposit', () => {
  // every creditDeposit path calls _increaseLedgerTotal after the
  // balance write. The helper guards on ROSE_TOKEN_ID, so non-ROSE tokens leave
  // _ledgerTotal untouched.
...
describe('AccountingLedgerTotal — BridgeAsset rejection in lock paths', () => {
  ...
  before(async () => {
    ...
    // Lock selectors live on the Accounting contract, so a single handle
    // exposes them at the proxy.
