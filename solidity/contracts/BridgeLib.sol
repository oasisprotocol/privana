 ...
 * @dev    Internal library holding the bridge dispatch and signing logic;
 *         `Accounting.resolveBridgeWithdrawal` is a thin wrapper over the
 *         storage choreography (queue lookup + idempotent emit).
 */
library BridgeLib {
     ...
     * @param userAddress         EIP-712 signer.
     ...
     *      `(destChainId, destTxNonce, routeAddress, maxGasCost)`;
     *      `accountingProxy` + `sapphireChainId` namespace the
    ...
    function resolveSign(
        ...
        uint256 sapphireChainId,
        uint256 index
    ) internal view returns (bytes memory signedTx, uint256 destChainId) {
        uint64 destTxNonce;
        address routeAddress;
        uint256 maxGasCost;
