...
import {HistoryKind, TokenType} from "../Types.sol";
import {IAccountingSiweAuth} from "../interfaces/IAccountingSiweAuth.sol";
import {UUPSUpgradeable} from "@openzeppelin/contracts-upgradeable/proxy/utils/UUPSUpgradeable.sol";

/**
 * @title MockAccountingPrevious
 * @notice Frozen storage layout of the previously deployed Accounting
 *         implementation, for proving that history entries written before an
 *         upgrade stay readable afterwards.
 * @dev Tests deploy this layout, seed history, then upgrade the proxy to the
 *      new implementation and read the entries back. The variable order below
 *      must mirror the deployed Accounting exactly; scripts/validate-upgrade.ts
 *      checks upgrades from this layout.
 */
contract MockAccountingPrevious is EIP712SignatureVerifier, EVMSignerAndVerifier, UUPSUpgradeable {
    ...
    // Frozen pre-upgrade copies of the Types.sol structs this layout embeds,
    // so later Types.sol changes cannot silently rewrite the snapshot.
    struct TokenInfo {
        TokenType tokenType;
        bytes data;
    }
