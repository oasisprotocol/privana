...
contract ProvethVerifier {
    ...
    function validateTxProof(
        ...
        RLPReader.RLPItem[] memory blockHeader = txProof
            ...
            .toRlpItem()
            .toList();
        bytes32 txRoot = bytes32(
            blockHeader[BLOCK_HEADER_TX_ROOT_INDEX].toUint()
        );
    ...
    function validateStorageProof(
        ...
        RLPReader.RLPItem[] memory blockHeader = storageProof
            .rlpBlockHeader
            .toRlpItem()
            .toList();
        bytes32 stateRoot = bytes32(
            blockHeader[BLOCK_HEADER_STATE_ROOT_INDEX].toUint()
        );

        // The key in the trie is the keccak256 of the address
        // We must convert the bytes of the keccak256 to nibbles for validateMPTProof
        bytes memory accountKey = decodeNibbles(
            bytes.concat(keccak256(bytes.concat(bytes20(storageProof.addr)))),
            0
        );
        bytes memory accountRlp = validateMPTProof(
            ...
            accountKey,
            RLPReader.toList(
                RLPReader.toRlpItem(storageProof.accountProofStack)
            )
        );
        RLPReader.RLPItem[] memory account = accountRlp.toRlpItem().toList();
        bytes32 storageRoot = bytes32(
            account[ACCOUNT_STORAGE_ROOT_INDEX].toUint()
        );

        // Prove storage proof
        bytes memory storageKey = decodeNibbles(
            bytes.concat(
                keccak256(bytes.concat(bytes32(storageProof.storageSlot)))
            ),
            0
        );
        bytes memory storageValue = validateMPTProof(
            ...
            storageKey,
            RLPReader.toList(
                RLPReader.toRlpItem(storageProof.storageProofStack)
            )
        );
    ...
    function validateMPTProof(
        ...
        for (uint i = 0; i < stack.length; i++) {
            ...
            if (node.length == 2) {
                ...
                uint prefixLength = sharedPrefixLength(
                    mptKeyOffset,
                    mptKey,
                    nodeKey
                );
