...
abstract contract EVMSignerAndVerifier is Initializable, OwnableUpgradeable {
    ...
    // `generateDepositAddressTransfer` and `generateDepositAddressERC20Transfer`
    // were consolidated into `Accounting.executeEmergencyWithdraw` — both branches
    // now share a single `EIP155Signer.sign` call site.
