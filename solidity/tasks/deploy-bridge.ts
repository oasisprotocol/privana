// CreateX salt helpers + minimal ABI for Rose bridge Phase 0 deployments.
//
...
// function `_guard`. Only the branches Phase 0 uses are reproduced — every
...
// Settled decision 7 (master plan): testnet default mint/burn cap for ROFLBridge
// on xROSE is 250_000e18. Operators may override via --mint-limit / --burn-limit.
...
/**
 * Mirror of CreateX `_guard` for the two permissioned branches Phase 0 uses.
 *
  ...
  if (first20 !== deployerAddr) {
    throw new Error(
      `createXGuardedSalt: unsupported salt layout. ` +
        ...
        `Only permissioned salts (sender-bytes == deployer) are supported in Phase 0.`,
    );
  }
...
// ─── deployment manifest ────────────────────────────────────────────────────
//
// Schema mirrors `solidity/deployments/bridge-<network>.json` as defined in
// docs/plans/rose-bridge-phase0/05-createx-deploy/07-write-deployment-manifest.md.
// The file is written by step 05.07; 05.02 only *reads* it (if present) and
// compares predicted values against it. A missing manifest means "first deploy
// of this network" — predictions become the values 05.07 will record.
  ...
  if (code === "0x" || code === "") {
    throw new Error(
      `preflight: CreateX not deployed at ${address}. ` +
        `Are you on the right network? Phase 0 requires CreateX as pre-existing infra.`,
    );
  }
}

// This check proves raw ≠ guarded but does not on its own prove "the deploy
// task chose the guarded prediction." The chain that closes the gap:
//   1. `assertHelperRequired` ensures raw and guarded would yield different
//      addresses (here).
//   2. `deployCreate3IfMissing` re-reads `getCode(predictedAddress)` after the
//      broadcast and throws if no code landed — a wrong prediction (e.g.
//      passing the raw-salt prediction) would fail this check because the
//      actual deploy lands at the guarded prediction.
//   3. The post-deploy parity check in the task body (step 16) compares
//      `xroseResult.address` against the predicted address one more time.
//   4. `test/CreateXAddress.ts` separately verifies the off-chain salt
//      builders mirror the upstream Solidity `_guard` formula.
...
// "someone edited the vendored Solidity after pinning". 04.03's verification
// script covers the network roundtrip; we only need the local check here.
...
// ROFLBridge) needs *before* talking to CreateX:
//   - xROSE: raw+guarded salt, initcode, runtime hash.
//   - ROFLBridge: raw+guarded salt only — its initcode requires the *predicted*
//     xROSE address, which the task body builds after `computeCreate3Address`.
//
...
// Read-back guard for the five values 05.05 must agree on: bridge mint/burn
...
task("deploy-bridge", "Preflight + CREATE3 deploy of XRose and ROFLBridge")
  ...
  .setAction(async (args, hre) => {
    ...
    const predicted: Partial<BridgeManifest> = {
      ...
      // `xroseRuntimeHash` omitted: the manifest stores the on-chain hash (step 16),
    ...
    console.log(
      manifest
        ...
        : `no manifest at ${manifestPath(hre.network.name)} — values above will be captured by 05.07.`,
    );
