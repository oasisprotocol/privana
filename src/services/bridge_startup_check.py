...
async def verify_bridge_runtime(
    ...
    # Defensive: if a future change drops the bridge surface from the
    # Accounting ABI, every bridge call would revert at runtime instead of
    # failing fast here at startup.
    fn_names = {
        name
    ...
    for required in (
        ...
        if required not in fn_names:
            raise BridgeStartupCheckError(
                ...
                "the bridge surface may be absent from the Accounting ABI."
            )
    ...
    logger.info(
        "Bridge runtime checks OK: rose_token_id=%s, roflBridge=%s, xrose=%s, custody=%s",
