...
def destination_chain_ids(settings: Settings) -> set[int]:
    """Destination set: every RPC-configured chain except Sapphire.

    Loose coupling — assumes any non-Sapphire RPC chain is a bridge
    destination (true while Base Sepolia is the only one). Switch to reading
    the on-chain ``roflBridgeAddress`` mapping when that no longer holds.
    """
