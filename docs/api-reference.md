...
- **The deployment chooses the credit card provider.** `ONRAMP_PROVIDER` must be configured explicitly; Testnet selects `transak`, localnet selects `moonpay`, and Mainnet leaves it unset. Clients cannot select a provider. Launch endpoints enforce the choice; exact authenticated recovery can still dispatch a known signed intent to its encoded provider.
...
- **Quote default.** `POST /onramp/session` accepts `default_crypto_amount`, an optional positive, finite JSON number in token units, forwarded as Transak's editable `defaultCryptoAmount`. Omit it or use `null` for Transak's default. It does not guarantee the amount received.
...
- **Configuration fails closed.** Startup fails if any configured intent key cannot be derived from ROFL. Invalid Transak configuration disables only its endpoints with `503`. The provider flip additionally requires registered token configuration and a trusted client-IP source (see next note).
- **The widget session requires one trusted client IP.** `TRANSAK_CLIENT_IP_MODE=header` (default) reads exactly one proxy-owned header (`TRANSAK_CLIENT_IP_HEADER`) whose browser-supplied value the proxy overwrites. `TRANSAK_CLIENT_IP_MODE=attested` instead requires an `ip_attestation` object in the `POST /onramp/session` body: an edge Worker signs `v1|referrer_domain|sha256(transaction_id)|ip|iat|exp|nonce` with the shared `TRANSAK_IP_ATTESTATION_SECRET`; the backend verifies the HMAC, the at-most-90-second window, single use of the nonce (best-effort, per backend process — the deployment runs one), and that the IP is global (rejecting the Cloudflare Worker source range), then forwards it as `x-user-ip`. Only the launch endpoint needs the attestation; intent minting and pending recovery do not.
