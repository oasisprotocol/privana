...
# live in test/py/test_rose_bridge_withdrawal.py — they exercise the
# kind-routed dispatcher that replaced the closure-based preflight.
...
# The "QUEUED record without live preflight → AWAITING_CLEAR" hazard is
# no longer reachable: the kind-routed dispatcher reconstructs every
# bridge preflight from persisted record fields, so a process restart
# between enqueue and broadcast does not lose the policy gate.
