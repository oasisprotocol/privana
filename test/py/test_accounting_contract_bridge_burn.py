...
`get_bridge_burn_nonce` reads via the typed `getBridgeBurnRequest` getter on the
Accounting contract — O(1) and not subject to Sapphire's 100-block `eth_getLogs`
cap. `list_bridge_burn_reservations` uses an event scan, paginated through
`paginated_get_logs`.
"""
